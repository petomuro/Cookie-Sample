...
const cookieRef: Ref<Cookie> = ref(Cookie);

// Functions
const openCookie = () => {
  cookieRef.value.openCookie();
};
</script>

<template>
  <Button :label="isCustomized ? 'Show cookie' : 'Zobraziť cookie'" @click="openCookie()"/>

  <Cookie v-if="isCustomized" ref="cookieRef">
    <template #header>
      {{ data.title }}
    </template>
    <template #default>
    ...
    <template #footer-buttons>
      <div v-for="item in data.buttonData" :key="item.id" class="p-1">
        <Button v-if="!item.isToggled" :label="item.title"/>
      </div>
    </template>
  </Cookie>
  <Cookie v-else ref="cookieRef"/>
</template>
